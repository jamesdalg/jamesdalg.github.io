...
y <- rnorm(200)
s <- subplot(
  plot_ly(data = noaa_clean, x = ~snow, type = "violin", side = "positive"),
  plotly_empty(),
  plot_ly(data = noaa_clean, x = ~snow, y = ~snwd, type = "histogram2dcontour"),
  plot_ly(data = noaa_clean, y = ~snwd, type = "violin", side = "positive"),
...
#library(rnoaa) #load NOAA package
#stations <- ghcnd_stations() #Grabs station ids
#nystationids <-  stations %>%  #filters stations for New York only.
#  filter(state == "NY") %>% 
#  distinct(id)
#nydat <- meteo_pull_monitors(nystationids$id, 
#                             date_min = "1981-01-01", 
#                             date_max = "2010-12-31", 
#                             var = c("PRCP", "SNOW", "SNWD", "TMAX", #"TMIN","LATITUDE","LONGITUDE","ELEVATION"))
#dir.create("data")
#saveRDS(nydat,file = "./data/nydat_latlng.rds")
noaa_latlng <- readRDS("./data/nydat_latlng.rds") %>% 
  as.tibble() %>%  #convert to tibble format.
  janitor::clean_names() %>%  #convert to snake case.
mutate(day = lubridate::day(date), #create day, month, and year variables.
