...
### Ranked Snowdepth for specific NOAA stations, displaying NY counties.
...
ny_county_data <- map_data('county') %>%
  ...
  group_by(group)  
tooltip <- str_c("Snowdepth: ",noaa_latlng %>% pull(snwd), "mm")
plotly::plot_ly(data = ny_county_data, x = ~long, y = ~lat) %>% 
add_polygons(size = I(2), fillcolor = ~subregion, text = ~subregion, showlegend = FALSE) %>% 
